...
import { RoutePlanner, type CalculatedRoutes } from '@/components/route-planner'
...
interface RoutePoint {
  ...
  type: 'port' | 'zone' | 'vessel' | 'custom' | 'city'
}

export default function MapasPage() {
  const [selectedDate, setSelectedDate] = useState('week')
  const [selectedZone, setSelectedZone] = useState('all')
  const [routeData, setRouteData] = useState<CalculatedRoutes | null>(null)
  const [origin, setOrigin] = useState<RoutePoint | null>(null)
  const [destination, setDestination] = useState<RoutePoint | null>(null)

  const handleRouteCalculated = (data: CalculatedRoutes) => setRouteData(data)
  const handleOriginChange = (point: RoutePoint | null) => { setOrigin(point); if (!point) setRouteData(null) }
  const handleDestinationChange = (point: RoutePoint | null) => { setDestination(point); if (!point) setRouteData(null) }

  const selectedRoute = routeData?.routes.find(r => r.id === routeData.selectedRoute)
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {selectedRoute && (
            <Badge className="bg-success/20 text-success border-0 ml-auto">
              {selectedRoute.label}: {selectedRoute.distance} mn
            </Badge>
          )}
        </div>
            ...
            <Card className="bg-card border-border h-full">
              ...
              <CardContent>
                <HeatmapMap 
                  height="500px" 
                  showRoute={!!routeData}
                  allRoutes={routeData?.routes || null}
                  selectedRouteId={routeData?.selectedRoute || 'optimal'}
                  optimizedRoute={null}
                ...
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-2 rounded-lg bg-secondary/50 text-center">
                      <p className="text-xl font-bold text-foreground">{selectedRoute.distance}</p>
                      <p className="text-[10px] text-muted-foreground">Millas náuticas</p>
                    </div>
                    <div className="p-2 rounded-lg bg-secondary/50 text-center">
                      <p className="text-xl font-bold text-foreground">{selectedRoute.estimatedTime}h</p>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="flex-1 h-2 bg-secondary rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${selectedRoute.efficiency}%` }}
                        />
                      </div>
                      <span className="text-sm font-bold text-success">{selectedRoute.efficiency}%</span>
                    </div>
                  </div>

                  <div className="space-y-1 text-xs">
                    <p className="text-muted-foreground">Waypoints de la ruta:</p>
                    {(routeData?.waypoints ?? []).map((wp, i, arr) => (
                      <div key={wp.id} className="flex items-center gap-2">
                        <div className={cn(
                          'w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold',
                          i === 0 ? 'bg-info/20 text-info' :
                          i === arr.length - 1 ? 'bg-success/20 text-success' :
                          'bg-accent/20 text-accent'
                        )}>
                          {i === 0 ? 'A' : i === arr.length - 1 ? 'B' : String(i)}
                        </div>
