...
function HeatmapLayer({ points }: { points: [number, number, number][] }) {
  ...
  useEffect(() => {
    const loadHeatmap = async () => {
      ...
      // leaflet.heat agrega heatLayer a L después del import
      const heatFn = L.heatLayer || (L.default && L.default.heatLayer)
      if (!heatFn) { console.warn('leaflet.heat no disponible'); return }
...
interface HeatmapMapProps {
  ...
  selectedRouteId?: 'optimal' | 'alt1' | 'alt2'
...
export function HeatmapMap({
  ...
  // Si hay 3 rutas, usar allRoutes; si no, fallback a optimizedRoute
  const routeToShow = allRoutes ? null : (optimizedRoute || (showRoute ? optimalRoute : null))
  ...
  return (
    ...
    >
      ...
      {/* ── 3 RUTAS simultáneas estilo Google Maps ── */}
      {allRoutes && allRoutes.map((route) => {
        const isSelected = route.id === selectedRouteId
        const pos = route.points.map(p => [p.lat, p.lng]) as LatLngExpression[]
        return (
          <React.Fragment key={route.id}>
            {/* Casing blanco (solo ruta seleccionada) */}
            {isSelected && (
              <Polyline positions={pos} pathOptions={{ color: '#ffffff', weight: 10, opacity: 0.85, lineCap: 'round', lineJoin: 'round' }} />
            )}
            {/* Línea principal */}
            <Polyline
              positions={pos}
            ...
            />
            {/* Etiqueta en el punto medio de cada ruta */}
            {route.points.length > 2 && (() => {
              const mid = route.points[Math.floor(route.points.length / 2)]
              return (
                <Marker
                  position={[mid.lat, mid.lng] as LatLngExpression}
                  icon={divIcon({
                    html: `<div style="background:${route.color};color:white;padding:2px 7px;border-radius:12px;font-size:10px;font-weight:700;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.4);border:2px solid white;">${route.distance} mn · ${route.estimatedTime}h</div>`,
                    className: '',
                    iconAnchor: [40, 10],
                  })}
                >
                  <Popup>
                    ...
                    <p className="text-xs">{route.distance} mn · {route.estimatedTime}h</p>
                  </Popup>
                </Marker>
              )
            })()}
      ...
      {/* Ruta única (fallback cuando no hay allRoutes) */}
      {routeToShow && routeToShow.length > 1 && (
        <>
          <Polyline positions={routeToShow.map(p => [p.lat, p.lng]) as LatLngExpression[]} pathOptions={{ color: '#ffffff', weight: 9, opacity: 0.9, lineCap: 'round', lineJoin: 'round' }} />
          <Polyline positions={routeToShow.map(p => [p.lat, p.lng]) as LatLngExpression[]} pathOptions={{ color: '#14b8a6', weight: 5, opacity: 1, lineCap: 'round', lineJoin: 'round' }} />
        </>
      )}
